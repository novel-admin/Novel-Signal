...
class AmazonSpApiClient:
    ...
    async def fetch(self, request: SyncRequest) -> tuple[RawSourcePage, ...]:
        """Keep the SourceAdapter contract without implying collection exists yet."""
        del request
        raise AmazonSpApiUnsupportedOperationError(
            "Amazon SP-API collection is not implemented in the connection-verification slice."
        )
    ...
    async def _marketplace_participations(self, access_token: str) -> httpx.Response:
        ...
        try:
            ...
            raise AmazonSpApiNetworkError("Amazon SP-API verification request timed out.") from exc
        except httpx.RequestError as exc:
            raise AmazonSpApiNetworkError("Amazon SP-API verification request failed.") from exc
        ...
        if response.is_error:
            raise AmazonSpApiError(
                f"Amazon SP-API verification request failed with status {response.status_code}."
            )
        return response
