  ...
  return <>
    ...
    <div className="universe-panel">
      ...
      {error ? <WorkError message={error} /> : sources === null || readiness === null ? <WorkLoading /> : !sources.length ? <WorkEmpty message="No source definitions are registered." /> : <>
        ...
        <h2 className="section-heading">Collection dependencies</h2><div className="rank-summary">{(["postgres", "object_store", "scheduler"] as const).map((key) => <Card key={key} label={key === "scheduler" ? "database scheduler" : key.replaceAll("_", " ")} value={`${readiness[key].status}${readiness[key].detail ? ` · ${readiness[key].detail}` : ""}`} />)}</div>
      </>}
    </div>
  </>;
}
